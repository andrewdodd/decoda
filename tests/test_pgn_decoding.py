...
class TestStartPositionValueExtractions:
    def test_it_extracts_whole_bytes(self):
        ...
        assert (0x3412, 1) == extract_value_at_location(inp, "1", BitLength(16))
        assert (0x5634, 2) == extract_value_at_location(inp, "2", BitLength(16))
    ...
    def test_it_extracts_fractions_of_a_byte(self):
        ...
        assert (0b1001, 0) == extract_value_at_location(inp, "1.2", BitLength(4))
        assert (0b0100, 0) == extract_value_at_location(inp, "1.3", BitLength(4))
        assert (0b010, 0) == extract_value_at_location(inp, "1.4", BitLength(3))
    ...
    def test_it_extracts_across_byte_boundaries(self):
        ...
        assert (0b0011010000, 1) == extract_value_at_location(inp, "1.7", BitLength(10))
        assert (0b001101000, 1) == extract_value_at_location(inp, "1.8", BitLength(9))
    ...
    def test_it_extracts_across_bytes_where_last_position_is_on_bit_1(self):
        ...
        #                                              X----X
        assert (0b001, 4) == extract_value_at_location(inp, "4.7-5.1", BitLength(3))
    ...
    def test_it_extracts_when_non_contiguous(self):
        ...
        #                         X--X       X------X
        assert (0x312, 1) == extract_value_at_location(inp, "1, 2.5", BitLength(12))

        # Example PGN 5552, SPN 1640,  12-bits, start_pos: '1, 2.6'
        # Byte :     2    1
        #         0x34 0x12 =>  0b00110100 0b00010010
        #                         X-X        X------X
        assert (0x112, 1) == extract_value_at_location(inp, "1, 2.6", BitLength(11))
    ...
    def test_only_2_sections_are_allowed(self):
        ...
        assert str(e.value) == "Should not have more than 2 sections in location spec"
    ...
    def test_fractional_bits_in_first_part(self):
        ...
        inp = (0x123456789ABCDEF0).to_bytes(8, "big")
        assert (0x0563, 2) == extract_value_at_location(inp, "2.5, 3", BitLength(12))

        # Example PGN 10240, SPN 10787,  12-bits, start_pos: '4, 5.1'
        inp = (0x123456789ABCDEF0).to_bytes(8, "big")
        assert (0x0A78, 4) == extract_value_at_location(inp, "4, 5.1", BitLength(12))
...
class TestStarDelimitedPGNs:
    ...
    def test_it_decodes_pgn_65260(self, inp: str, expected: str, pgn_65260: PGN):
...
class TestTransportProtocol:
    @pytest.mark.parametrize("control_byte", [0, 1, 2, 15, 18, 20, 31, 33, 254])
    ...
    def test_it_decodes_pgn_60416_CTS(self, pgn_60416: PGN):
        ...
        assert str(results[1]) == "SPN:2557: Total Message Size (TP.CM_RTS) = 62193"
        assert str(results[2]) == "SPN:2558: Total Number of Packets (TP.CM_RTS) = 243"
        assert (
            str(results[3]) == "SPN:2559: Maximum Number of Packets (TP.CM_RTS) = 244"
        )
        assert (
            str(results[4])
    ...
    def test_it_decodes_pgn_60416_BAM(self, pgn_60416: PGN):
        ...
        assert len(results) == 4
        assert str(results[1]) == "SPN:2567: Total Message Size (TP.CM_BAM) = 62193"
        assert str(results[2]) == "SPN:2568: Total Number of Packets (TP.CM_BAM) = 243"
        assert (
            str(results[3])
